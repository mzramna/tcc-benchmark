...
try:
    f = open("teste_tempo_aditional_data_arm64.json", "r")
    loaded=json.load(f)
    f.close()
except:
    loaded={}
...
while loaded["quantidade_elementos"]<maximo_elementos:
    ...
    while len(loaded["tempos"][-1])<total_por_ciclo:
        timer=Timer()
        f = open("teste_tempo_execucao_arm64.log", "a")
        try:
        ...
        f.write(str(duracao)+"\n")
        f.close()
        f = open("teste_tempo_aditional_data_arm64.json", "w")
...
os.remove("teste_tempo_aditional_data_arm64.json")
